...
const char mqtt_name[] = "client_name";
...
void sendToServer() {
  if (currentMillis - previoussendToServer >= sendToServerInterval) {
    ...
    Serial.println("MQTT");
    char c_wifi[12];
    sprintf(c_wifi, "%d", WiFi.RSSI());
    char c_co2[12];
    sprintf(c_co2, "%d", Co2);
    char c_pm01[12];
    sprintf(c_pm01, "%d", pm01);
    char c_pm25[12];
...
//      (pm03PCount < 0 ? "" : ", \"pm003_count\":" + String(pm03PCount)) +
    ...
    if (WiFi.status() == WL_CONNECTED) {
      ...
      if(!mqtt_client.connected())
      {
        ...
        {
          Serial.print("failed! State: "); Serial.println(mqtt_client.state());
          delay(2000);
        }
      }
      else
      {
        char tmp_topic[TOPIC_MAXLEN];
        int16_t size = sizeof(tmp_topic) - (sizeof(mqtt_topic) - 1);
        if(size < 6)
        {
          Serial.println("Topic Max Length too small...");
          return;
        }
        uint8_t start = (sizeof(mqtt_topic) - 1);
        strlcpy(tmp_topic, mqtt_topic, TOPIC_MAXLEN);

        strlcpy((tmp_topic + start), "/wifi", size);
        mqtt_client.publish(tmp_topic, c_wifi);
        strlcpy((tmp_topic + start), "/co2", size);
        mqtt_client.publish(tmp_topic, c_co2);
        strlcpy((tmp_topic + start), "/pm01", size);
        mqtt_client.publish(tmp_topic, c_pm01);
        strlcpy((tmp_topic + start), "/pm25", size);
        ...
        mqtt_client.publish(tmp_topic, c_nox);
        strlcpy((tmp_topic + start), "/temp", size);
        mqtt_client.publish(tmp_topic, c_temp);
        strlcpy((tmp_topic + start), "/hum", size);
        mqtt_client.publish(tmp_topic, c_hum);
      }
      #endif
      #ifdef CLOUD
